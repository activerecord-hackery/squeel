shared_examples "Relation on ActiveRecord 4.2" do
  describe 'finding by attribute' do
    ...
    it 'casts an empty string to the proper value' do
      ...
      queries.first.should match /"people"."id" = ?/
    end
  ...
  describe '#build_arel' do
    ...
    it 'reverses order of Arel::Attributes::Attribute when #last is called' do
  ...
  describe '#includes' do
    ...
    it 'includes a keypath' do
      relation = Note.includes{notable(Article).person.children}.references(:all).
        where{notable(Article).person.children.name == 'Ernie'}

      relation.debug_sql.should match /FROM "notes" LEFT OUTER JOIN "articles" ON "articles"."id" = "notes"."notable_id" AND "notes"."notable_type" = 'Article' LEFT OUTER JOIN "people" ON "people"."id" = "articles"."person_id" LEFT OUTER JOIN "people" "children_people" ON "children_people"."parent_id" = "people"."id" WHERE "children_people"."name" = 'Ernie'/
    end
  ...
  describe '#group' do
    ...
    it 'work with joined tables' do
      relation = Person.joins{articles}.group{articles.title}.having{{articles => {max(id) => id}}}
      relation.to_sql.should match /SELECT "people".* FROM "people" INNER JOIN "articles" ON "articles"."person_id" = "people"."id" GROUP BY "articles"."title" HAVING max\("articles"."id"\) = "articles"."id"/
    end
  ...
  describe '#where' do
    ...
    it 'maps conditions onto their proper table with multiple polymorphic joins' do
      relation = Note.includes{ [notable(Article), notable(Person)] }.references(:notables)
      ...
      people_notes.should have(9).items
      article_notes.should have(30).items
      people_and_article_notes.should have(39).items
    end
    ...
    it 'allows hash equality conditions against a polymorphic belongs_to with an AR::Base value' do
      ...
      relation.to_sql.should match /"notes"."notable_id" = #{first_person.id}/
    end
  ...
  describe '#debug_sql' do
    ...
    it 'returns the query that would be run against the database, even if eager loading' do
      relation = Person.includes(:comments.outer, :articles.outer).
  ...
  describe '#merge' do
    ...
    it 'uses the given equality condition in the case of a conflicting where from a default scope' do
      if activerecord_version_at_least '3.1'
        relation = PersonNamedBill.rewhere(name: 'Ernie')
        sql = relation.to_sql
    ...
    it "doesn't merge the default scope more than once" do
      relation = PersonNamedBill.all.highly_compensated.ending_with_ill
      sql = relation.to_sql
    ...
    it "doesn't hijack the table name when merging a relation with different base and default_scope" do
      relation = Article.joins(:person).merge(PersonNamedBill.all)
      sql = relation.to_sql
      sql.scan(/"people"."name" = 'Bill'/).should have(1).item
    end

    it 'merges scopes that contain functions' do
      relation = PersonNamedBill.all.with_salary_equal_to(100)
      sql = relation.to_sql
